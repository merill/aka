...
async function main() {
  ...
  // A representative icon per category: the one most of its entries use.
  // Gives the pickers a real product glyph for categories akams.fyi has no
  // local SVG for (Purview, XDR Sentinel, Licensing…).
  const catIcons = {};
  for (const e of entries) {
    const c = normalizeCategory(e.category);
    if (!c || !e.iconUrl) continue;
    catIcons[c] = catIcons[c] || {};
    catIcons[c][e.iconUrl] = (catIcons[c][e.iconUrl] || 0) + 1;
  }
  const iconForCategory = (label) => {
    const tally = catIcons[label];
    if (!tally) return '';
  ...
  const categories = ordered.map((label) => {
    ...
    return iconUrl ? { value, label, iconUrl } : { value, label };
  });
